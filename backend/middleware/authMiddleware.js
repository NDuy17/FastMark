...
/** Cho phép user bị khóa (Status=0) — dùng cho /me và khiếu nại khóa nick. */
async function verifyFirebaseTokenAllowBlocked(req, res, next) {
  const header = req.headers.authorization || "";
  const [scheme, token] = header.split(" ");
...
module.exports.verifyFirebaseTokenAllowBlocked = verifyFirebaseTokenAllowBlocked;
